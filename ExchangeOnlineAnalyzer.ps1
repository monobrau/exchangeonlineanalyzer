...
Safe-ImportModule -ModulePath "$PSScriptRoot\Modules\ConnectWiseManage.psm1"
...
# Graph App (App-Only) Section
$graphAppSectionTitle = New-Object System.Windows.Forms.Label
$graphAppSectionTitle.Text = "Graph App (App-Only)"
$graphAppSectionTitle.Font = New-Object System.Drawing.Font('Segoe UI', 10, [System.Drawing.FontStyle]::Bold)
$graphAppSectionTitle.Location = New-Object System.Drawing.Point(10,615)
$graphAppSectionTitle.AutoSize = $true

$chkPromptToCreateGraphApp = New-Object System.Windows.Forms.CheckBox
$chkPromptToCreateGraphApp.Text = "Prompt to create Graph app when pulling reports for a tenant without one (default: off)"
$chkPromptToCreateGraphApp.Location = New-Object System.Drawing.Point(10,640)
$chkPromptToCreateGraphApp.AutoSize = $true
$chkPromptToCreateGraphApp.Width = 550
$chkPromptToCreateGraphAppTooltip = New-Object System.Windows.Forms.ToolTip
$chkPromptToCreateGraphAppTooltip.SetToolTip($chkPromptToCreateGraphApp, "When enabled, first report pull for a tenant without Graph app credentials will prompt to create one and save to Windows Credential Manager")

$btnCreateGraphApp = New-Object System.Windows.Forms.Button
$btnCreateGraphApp.Text = "Create Graph App"
...
$btnCreateGraphApp.add_Click({
    $launcherPath = Join-Path $PSScriptRoot "Start-NewGraphInboxRulesApp.ps1"
...
$btnDeleteGraphApp = New-Object System.Windows.Forms.Button
$btnDeleteGraphApp.Text = "Delete Graph App"
$btnDeleteGraphApp.Location = New-Object System.Drawing.Point(495, 638)
$btnDeleteGraphApp.Size = New-Object System.Drawing.Size(125, 28)
$btnDeleteGraphApp.BackColor = [System.Drawing.Color]::FromArgb(198, 40, 40)
$btnDeleteGraphApp.ForeColor = [System.Drawing.Color]::White
$btnDeleteGraphAppTooltip = New-Object System.Windows.Forms.ToolTip
$btnDeleteGraphAppTooltip.SetToolTip($btnDeleteGraphApp, "Remove app registration in Entra and delete stored credentials per tenant. For credentials only (no Entra change), use Clear local WCM.")
$btnClearLocalGraphCreds = New-Object System.Windows.Forms.Button
$btnClearLocalGraphCreds.Text = "Clear local WCM"
$btnClearLocalGraphCreds.Location = New-Object System.Drawing.Point(625, 638)
$btnClearLocalGraphCreds.Size = New-Object System.Drawing.Size(128, 28)
$btnClearLocalGraphCredsTooltip = New-Object System.Windows.Forms.ToolTip
$btnClearLocalGraphCredsTooltip.SetToolTip($btnClearLocalGraphCreds, "Remove Graph app credentials from Windows Credential Manager on this PC only. Does not delete the app in Entra. Lists tenant IDs even when the name is unknown.")
$btnClearLocalGraphCreds.add_Click({
    try {
        Import-Module (Join-Path $PSScriptRoot "Modules\GraphAppCredential.psm1") -Force -ErrorAction Stop
...
$btnExportAppCreds.Location = New-Object System.Drawing.Point(758, 638)
$btnExportAppCreds.Size = New-Object System.Drawing.Size(110, 28)
$btnExportAppCredsTooltip = New-Object System.Windows.Forms.ToolTip
$btnExportAppCredsTooltip.SetToolTip($btnExportAppCreds, "Export app credentials from WCM to an encrypted file. Optional: embed tenant display names (stored or from Graph) so Import on another PC shows friendly names without Graph.")
$btnExportAppCreds.add_Click({
    try {
        ...
        $chkEmbedNames.Text = "Embed tenant display names for other PCs (uses stored WCM names; if missing, calls Microsoft Graph per tenant — needs network and app permissions)."
        ...
        [System.Windows.Forms.MessageBox]::Show("Exported $($ids.Count) EOA credential(s) to $($sfd.FileName). Import this file on another PC with the same password; display names in the file are written into WCM there.", "Export", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
    } catch {
...
$btnImportAppCreds.Location = New-Object System.Drawing.Point(873, 638)
$btnImportAppCreds.Size = New-Object System.Drawing.Size(112, 28)
$btnImportAppCredsTooltip = New-Object System.Windows.Forms.ToolTip
$btnImportAppCredsTooltip.SetToolTip($btnImportAppCreds, "Import app credentials (and tenant display names, if present in the file) from an encrypted export into WCM on this PC.")
...
$btnTestManageConnection.add_Click({
    try {
        Import-Module (Join-Path $PSScriptRoot "Modules\ConnectWiseManage.psm1") -Force -ErrorAction Stop
...
$allControls = @($sTitle,$lblSettingsLocation,$txtSettingsLocation,$btnBrowseSettingsLocation,
    ...
    $graphAppSectionTitle,$chkPromptToCreateGraphApp,$btnCreateGraphApp,$btnDeleteGraphApp,$btnClearLocalGraphCreds,$btnExportAppCreds,$btnImportAppCreds,
    ...
    try {
        ...
        foreach ($searchTerm in $searchTerms) {
            ...
            try {
                ...
                $users1 = Get-MgUser -Filter "startsWith(DisplayName,'$searchTerm') or startsWith(UserPrincipalName,'$searchTerm')" -All -Property Id, UserPrincipalName, DisplayName, AssignedLicenses -ErrorAction SilentlyContinue
                ...
                $users2 = Get-MgUser -Filter "startsWith(DisplayName,'$searchTermLower') or startsWith(UserPrincipalName,'$searchTermLower')" -All -Property Id, UserPrincipalName, DisplayName, AssignedLicenses -ErrorAction SilentlyContinue
                $users3 = Get-MgUser -Filter "startsWith(DisplayName,'$searchTermUpper') or startsWith(UserPrincipalName,'$searchTermUpper')" -All -Property Id, UserPrincipalName, DisplayName, AssignedLicenses -ErrorAction SilentlyContinue
                $users4 = Get-MgUser -Filter "startsWith(DisplayName,'$searchTermTitle') or startsWith(UserPrincipalName,'$searchTermTitle')" -All -Property Id, UserPrincipalName, DisplayName, AssignedLicenses -ErrorAction SilentlyContinue
            ...
            if ($users.Count -eq 0) {
                ...
                try {
                    ...
                    $usersAlt1 += Get-MgUser -Filter "DisplayName eq '$searchTermLower'" -All -Property Id, UserPrincipalName, DisplayName, AssignedLicenses -ErrorAction SilentlyContinue
                    ...
                    $usersAlt2 += Get-MgUser -Filter "UserPrincipalName eq '$searchTermLower'" -All -Property Id, UserPrincipalName, DisplayName, AssignedLicenses -ErrorAction SilentlyContinue
    ...
    try {
        ...
        $generateButton.add_Click({
            ...
            try {
                ...
                # Build report selections early to determine required auth
                $signInLogsDays = 7
                ...
                # When only inbox rules selected: try WCM (app reg) first - Graph can fetch inbox rules, no Exchange needed
                $onlyInboxRules = $reportSelections.IncludeInboxRules -and -not $reportSelections.IncludeMessageTrace -and -not $reportSelections.IncludeTransportRules -and -not $reportSelections.IncludeMailFlowConnectors -and -not $reportSelections.IncludeMailboxForwarding -and -not $reportSelections.IncludeUnifiedAuditLogs
                if ($onlyInboxRules -and $required.NeedsExchange) {
                    $tenantId = $null
                    try { $ctx = Get-MgContext -ErrorAction SilentlyContinue; if ($ctx -and $ctx.TenantId) { $tenantId = $ctx.TenantId } } catch {}
                    if (-not $tenantId) { try { $conn = Get-ConnectionInformation -ErrorAction SilentlyContinue; $c = if ($conn -is [Array] -and $conn.Count -gt 0) { $conn[0] } else { $conn }; if ($c -and $c.TenantId) { $tenantId = $c.TenantId } } catch {} }
                    if (-not $tenantId) { try { $org = Get-OrganizationConfig -ErrorAction SilentlyContinue; if ($org -and $org.ExternalDirectoryOrganizationId) { $tenantId = $org.ExternalDirectoryOrganizationId } } catch {} }
                    if ($tenantId) {
                        $graphAppMod = Join-Path $PSScriptRoot "Modules\GraphAppCredential.psm1"
                ...
                # Pre-warm connections only for what the selected reports need
                $progressLabel.Text = "Validating connections..."
                [System.Windows.Forms.Application]::DoEvents()
                $graphTokenForReport = $null
                try {
                    ...
                    if ($required.NeedsGraph) {
                        $mgOk = $false
                        $tenantId = $null
                        try { $ctx = Get-MgContext -ErrorAction SilentlyContinue; if ($ctx -and $ctx.Account) { $mgOk = $true; $tenantId = $ctx.TenantId } elseif ($ctx -and $ctx.TenantId) { $tenantId = $ctx.TenantId } } catch {}
                        if (-not $tenantId) { try { $conn = Get-ConnectionInformation -ErrorAction SilentlyContinue; $c = if ($conn -is [Array] -and $conn.Count -gt 0) { $conn[0] } else { $conn }; if ($c -and $c.TenantId) { $tenantId = $c.TenantId } } catch {} }
                        if (-not $tenantId) { try { $org = Get-OrganizationConfig -ErrorAction SilentlyContinue; if ($org -and $org.ExternalDirectoryOrganizationId) { $tenantId = $org.ExternalDirectoryOrganizationId } } catch {} }
                        if ($tenantId) {
                            $graphAppMod = Join-Path $PSScriptRoot "Modules\GraphAppCredential.psm1"
                        ...
                        if (-not $mgOk) { try { $ctx = Get-MgContext -ErrorAction SilentlyContinue; if ($ctx -and $ctx.Account) { $mgOk = $true } } catch {} }
                        if (-not $mgOk -and $settings -and $settings.PromptToCreateGraphApp) {
                            $createResult = [System.Windows.Forms.MessageBox]::Show("No Graph connection and no app credentials for this tenant. Create a Graph app now and save to Windows Credential Manager? (Requires admin - Application.ReadWrite.All)", "Create Graph App?", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
                ...
                # Validate at least one report is selected (reportSelections built above)
                $anySelected = $false
                ...
                # Set log context so entries include company (and ticket if available) for multi-report log filtering
                ...
                if ($graphTokenForReport) { $reportParams['GraphAccessToken'] = $graphTokenForReport }
                $securityReport = New-SecurityInvestigationReport @reportParams
...
$bulkTenantExporterButton.add_Click({
    ...
    try {
        ...
        $bulkAnalyzeSingleBtn.add_Click({
            $defaultPath = if ($script:outputBaseDir -and (Test-Path $script:outputBaseDir)) { $script:outputBaseDir } else { Join-Path ([Environment]::GetFolderPath('MyDocuments')) "ExchangeOnlineAnalyzer\SecurityInvestigation" }
            $fbd = New-Object System.Windows.Forms.FolderBrowserDialog
...
try {
    ...
    if (Test-Path "`$ScriptRoot\Modules\GraphAppCredential.psm1") { Import-Module "`$ScriptRoot\Modules\GraphAppCredential.psm1" -Force -ErrorAction SilentlyContinue }
        ...
        Write-Host ""
                ...
                if (`$command -match '\|TICKET_DATA:(.+)$') {
                    try {
                        ...
                        Write-Host "Parsing ticket data JSON: `$ticketDataJson" -ForegroundColor Gray
...
"@
            ...
            $script:nextClientNumber = 1  # Track next available client number
            ...
            # Clear local WCM only (Entra app unchanged); shows tenant GUID when name unknown
            ...
            $script:clientValidatedUsers = @{}  # Store validated user UPNs per tenant (keyed by ClientNumber)
            ...
            # Function to search and validate users
            ...
            function Add-NewTenant {
                        ...
                        if (Test-Path $statusFilePath) {
                            try {
                                ...
                                if ($readyFound) {
                                    ...
                                    # When only Graph needed (e.g. inbox rules + app reg), auto-trigger Graph Auth
                                    $required = Get-CurrentRequiredAuth
                ...
                # Log out Graph only (worker session) — disconnect Graph + clear token cache; leaves Exchange as-is
                $logoutGraphBtn = New-Object System.Windows.Forms.Button
                $logoutGraphBtn.Text = "Log out Graph"
                $logoutGraphBtn.Location = New-Object System.Drawing.Point(860, ($yPos + 10))
                $logoutGraphBtn.Size = New-Object System.Drawing.Size(105, 30)
                ...
                $logoutGraphBtnTooltip.SetToolTip($logoutGraphBtn, "Disconnect Microsoft Graph in this client's PowerShell window and clear its Graph token cache. Exchange Online stays signed in. Use when Graph is stuck on the wrong tenant or before re-running Graph Auth.")

                # Reset Auth button
                $resetAuthBtn = New-Object System.Windows.Forms.Button
                $resetAuthBtn.Text = "Reset Auth"
                $resetAuthBtn.Location = New-Object System.Drawing.Point(970, ($yPos + 10))
                ...
                $appRegTenantLabel.Location = New-Object System.Drawing.Point(10, ($yPos + 50))
                ...
                $appRegTenantCombo.Location = New-Object System.Drawing.Point(95, ($yPos + 47))
                ...
                $useInteractiveGraphCheckBox.Location = New-Object System.Drawing.Point(325, ($yPos + 50))
                ...
                # Show/hide Graph/Exchange buttons based on report selections immediately
                ...
                $ticketTextBox.add_TextChanged({
                    try {
                        ...
                        $ticketContent = $this.Text
                        if ($script:clientAuthStates.ContainsKey($capturedClientNumForTicket)) {
                            $state = $script:clientAuthStates[$capturedClientNumForTicket]
                            if ($state.GraphAuthenticated -and $state.ExchangeAuthenticated -and
                                -not [string]::IsNullOrWhiteSpace($ticketContent) -and
                                $script:clientAuthControls[$capturedClientNumForTicket].ExtractEmailsButton) {
                                $script:clientAuthControls[$capturedClientNumForTicket].ExtractEmailsButton.Enabled = $true
                            }
                        }
                ...
                $toggleBtn.add_Click({
                    ...
                    if ($isExpanded) {
                        ...
                        # Show expanded controls (Graph/Exchange visibility based on report selections)
                    ...
                    if ($script:clientValidatedUsers.ContainsKey($clientNum)) {
                        ...
                        if (-not [string]::IsNullOrWhiteSpace($searchTerms)) {
                            ...
                            if ($ticketNumbers.Count -gt 0) {
                                ...
                                $ticketDataJson = @{
                                    TicketNumbers = $ticketNumsArray
                                    TicketContent = $filteredTicketContent
                                } | ConvertTo-Json -Compress
                                $command += "|TICKET_DATA:$ticketDataJson"
                            }
                            Send-CommandToSession -ClientNumber $clientNum -Command $command -TimeoutSeconds 10 | Out-Null
                ...
                $graphAuthBtn.add_Click({
                    ...
                    if ($response -like "GRAPH_AUTH_SUCCESS:*") {
                        ...
                        if ($required.NeedsExchange) {
                            ...
                            # Graph-only: show ticket controls (no Exchange Auth needed)
                ...
                $exchangeAuthBtn.add_Click({
                    ...
                    if ($response -like "EXCHANGE_AUTH_SUCCESS*") {
                        ...
                        # Show ticket controls and Generate Reports button (if required auth is complete)
                        $script:clientAuthControls[$clientNum].TicketLabel.Visible = $true
                        $script:clientAuthControls[$clientNum].TicketLabel.Enabled = $true
                        $script:clientAuthControls[$clientNum].TicketTextBox.Visible = $true
                        $script:clientAuthControls[$clientNum].TicketTextBox.Enabled = $true
                ...
                # Log out Graph only (worker disconnect + token cache clear; Exchange unchanged)
                $logoutGraphBtn.add_Click({
                    $clientNum = $this.Tag
                    if (-not $clientNum) { $clientNum = $capturedClientNum }
                    if (-not $script:clientAuthStates[$clientNum].GraphAuthenticated) { return }
                    ...
                    if ($resp -eq "GRAPH_DISCONNECT_SUCCESS") {
                        ...
                        if ($script:clientAuthControls[$clientNum].GraphButton -and -not $script:clientAuthControls[$clientNum].GraphButton.IsDisposed) {
                ...
                $resetAuthBtn.add_Click({
                    ...
                    # Reset UI controls (visibility based on report selections)
                    ...
                    if ($script:clientAuthControls[$clientNum].ExchangeButton) { $script:clientAuthControls[$clientNum].ExchangeButton.Text = "Exchange Online Auth" }
            ...
            # Old button handlers removed - now handled dynamically in Add-NewTenant function
                ...
                try {
                    ...
                    foreach ($clientNum in $script:clientAuthControls.Keys) {
                        try {
                            ...
                            # Silently ignore errors for individual clients
                        }
                    }
                } catch {
                    # Stop timer if any error occurs
                    try {
                        if ($statusUpdateTimer) {
                            $statusUpdateTimer.Stop()
                        }
        ...
        $bulkMainPanel.Controls.AddRange(@(
            ...
            $bulkStartButton, $bulkAnalyzeSingleBtn, $bulkAnalyzeAllBtn, $bulkProgressLabel, $bulkStatusTextBox, $bulkCloseButton
        ))
