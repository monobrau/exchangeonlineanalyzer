function Start-ExchangeOnlineAnalyzer {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [string]$AppRoot
    )
    $script:EOA_AppRoot = $AppRoot
...
Safe-ImportModule -ModulePath "$script:EOA_AppRoot\Modules\Connectors.psm1"
Safe-ImportModule -ModulePath "$script:EOA_AppRoot\Modules\SessionRevocation.psm1"
Safe-ImportModule -ModulePath "$script:EOA_AppRoot\Modules\SignInManagement.psm1"
Safe-ImportModule -ModulePath "$script:EOA_AppRoot\Modules\ExportUtils.psm1"
$reportAnalysisPath = Join-Path $script:EOA_AppRoot 'Modules\ReportAnalysis.psm1'
if (Test-Path $reportAnalysisPath) { Safe-ImportModule -ModulePath $reportAnalysisPath }
Safe-ImportModule -ModulePath "$script:EOA_AppRoot\Modules\EntraInvestigator.psm1"
Safe-ImportModule -ModulePath "$script:EOA_AppRoot\Modules\SecurityAnalysis.psm1"
# Import Settings module (needed for settings tab and configuration)
Safe-ImportModule -ModulePath "$script:EOA_AppRoot\Modules\Settings.psm1"
# Import SecurityHelpers for input validation and sanitization
$securityHelpersPath = Join-Path $script:EOA_AppRoot 'Scripts\Common\SecurityHelpers.psm1'
...
$aiSendBtn.add_Click({
    try {
        ...
        if ($provider -eq 'Gemini') {
            $scriptPath = Join-Path $script:EOA_AppRoot "Scripts\Send-To-Gemini.ps1"
            ...
            $scriptPath = Join-Path $script:EOA_AppRoot "Scripts\Send-To-Claude.ps1"
...
$btnDeleteGraphApp.add_Click({
    $scriptPath = Join-Path $script:EOA_AppRoot "Remove-GraphInboxRulesApp.ps1"
    if (-not (Test-Path $scriptPath)) {
        [System.Windows.Forms.MessageBox]::Show("Script not found: $scriptPath", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        return
    }
...
$settingsTab.add_Enter({
    try {
        ...
        $settingsModulePath = Join-Path $script:EOA_AppRoot "Modules\Settings.psm1"
...
$fixModsBtn.add_Click({
    try {
        Import-Module "$script:EOA_AppRoot\Modules\GraphOnline.psm1" -Force -ErrorAction SilentlyContinue
...
$entraSignInExportButton.add_Click({
    ...
    if (-not (Get-Command Get-EntraSignInLogs -ErrorAction SilentlyContinue)) {
        try {
            Import-Module "$script:EOA_AppRoot\Modules\EntraInvestigator.psm1" -Force -Global -ErrorAction Stop
        } catch {
    ...
    try {
        ...
        $generateButton.add_Click({
            ...
            try {
                ...
                Import-Module "$script:EOA_AppRoot\Modules\ExportUtils.psm1" -Force -ErrorAction Stop
                Import-Module "$script:EOA_AppRoot\Modules\Settings.psm1" -Force -ErrorAction SilentlyContinue
                ...
                $tenantName = $null
                try {
                    Import-Module "$script:EOA_AppRoot\Modules\BrowserIntegration.psm1" -Force -ErrorAction SilentlyContinue
...
$loadFirefoxUi = {
    try {
        ...
        Import-Module "$script:EOA_AppRoot\Modules\BrowserIntegration.psm1" -Force -ErrorAction Stop
...
$openSignInsBtn.add_Click({ try { Import-Module "$script:EOA_AppRoot\Modules\BrowserIntegration.psm1" -Force; Open-EntraDeepLink -ProfileName $profileCombo.SelectedItem -ContainerName $containerCombo.SelectedItem -Target 'SignIns' } catch {} })
$openCABtn.add_Click({ try { Import-Module "$script:EOA_AppRoot\Modules\BrowserIntegration.psm1" -Force; Open-EntraDeepLink -ProfileName $profileCombo.SelectedItem -ContainerName $containerCombo.SelectedItem -Target 'ConditionalAccess' } catch {} })
...
$entraRiskyUsersButton.add_Click({
    try {
        ...
        $securityAnalysisModule = Join-Path $script:EOA_AppRoot "Modules\SecurityAnalysis.psm1"
...
$entraFixModulesButton.add_Click({
    ...
    try {
        ...
        Import-Module "$script:EOA_AppRoot\Modules\GraphOnline.psm1" -Force -ErrorAction Stop
...
Export-ModuleMember -Function Start-ExchangeOnlineAnalyzer
